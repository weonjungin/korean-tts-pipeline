...
def kor_preprocess(text):
    ...
    return torch.from_numpy(sequence).long().to(device), sequence.shape[1]  
...
def synthesize(model, vocoder, text, sentence, dur_pitch_energy_aug, src_len, prefix=''):
    ...
    mel, mel_postnet, log_duration_output, f0_output, energy_output, _, _, mel_len = model(text, src_len, dur_pitch_energy_aug=dur_pitch_energy_aug, energy_stat=energy_stat)
...
if __name__ == "__main__":
    ...
    if mode != '4':
        for s in sentence:
            text, src_len = kor_preprocess(s)  
            synthesize(model, vocoder, text, s, dur_pitch_energy_aug, src_len, prefix='step_{}-duration_{}-pitch_{}-energy_{}'.format(args.step, dur_pitch_energy_aug[0], dur_pitch_energy_aug[1], dur_pitch_energy_aug[2]))
    else:
        text, src_len = kor_preprocess(sentence) 
        synthesize(model, vocoder, text, sentence, dur_pitch_energy_aug, src_len, prefix='step_{}-pitch_{}-energy_{}'.format(args.step, dur_pitch_energy_aug[0], dur_pitch_energy_aug[1], dur_pitch_energy_aug[2]))
